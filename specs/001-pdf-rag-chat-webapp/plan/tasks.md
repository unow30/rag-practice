...
- [x] 평가 스크립트 실행 (실제 PDF 업로드 후 수동 실행 필요)
- [x] 결과 기록 (각 지표 수치) → evaluation/BASELINE.md 업데이트
- [x] **판단**: Recall@5 = 0.00% < 80% → v1 진행
...
- [x] 평가 스크립트 재실행
- [x] v0 대비 Recall@5 변화 확인 (0% → 61.54%)
- [x] **판단**: Recall@5 = 61.54% < 80% → v2 진행
...
- [x] 평가 스크립트 재실행
- [x] 표현 불일치 카테고리 질문 Recall 개선 확인 (개선 없음 61.54%)
- [x] Latency 증가분 확인 (9,247ms, +718ms) → Multi-query 효과 없음 → v3 진행
...
- [x] 목표 지표 확인:
  - Recall@5 = 61.54% < 80% ❌
  - Answerable@5 = 76.67% < 80% ❌
  - Exact Match = 0.00% < 60% ❌
  - Partial Match = 85.00% ≥ 85% ✅
  - Latency (첫 토큰) = 22,294ms (초기 모델 로드 포함) ❌
- [x] 실패 카테고리 분석: TABLE 카테고리(Q6~Q9, Q12) — 이미지 기반 데이터, OCR 필요
- [x] 결과 `evaluation/results/final.json` 저장
