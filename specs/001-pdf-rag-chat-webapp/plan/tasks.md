...
- [ ] **사이드바**: 문서 업로드 (드래그 앤 드롭, 진행률), 문서 목록, 질의 대상 문서 선택, 삭제 버튼
- [ ] **메인 영역**: 대화창 (질문 입력, 답변 스트리밍 표시)
- [ ] 답변 하단에 출처 표시 (문서명, 페이지 번호, 인용 구절)
- [ ] 대화 기록 초기화 버튼
- [ ] 문서 처리 중 상태 폴링 (1초 간격)
- [ ] AI 오류 시 재시도 버튼 표시
...
- [ ] PDF 1개 업로드 → 30초 이내 READY 전환 확인
- [ ] 텍스트 질문 → 3초 이내 첫 토큰 + 출처 포함 답변
- [ ] 문서에 없는 내용 질문 → "찾을 수 없습니다" 응답
- [ ] 문서 삭제 후 해당 문서 내용 미반환 확인
- [ ] 50MB 초과 파일 업로드 → 오류 메시지
- [ ] 21번째 문서 업로드 → 한도 초과 메시지
